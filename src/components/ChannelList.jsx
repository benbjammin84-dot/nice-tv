...
export default function ChannelList({ source, activeCategory, onCategoryChange, onChannelSelect, activeChannel }) {
  const [search, setSearch] = useState('');

  const categories = useMemo(() => {
    if (!source?.channels) return [];
    const groups = ['All', ...new Set(source.channels.map(c => c.group))].filter(Boolean);
    return groups;
  }, [source]);
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {filtered.map((ch, i) => (
          <button
            ...
            className={`channel-card w-full flex items-center gap-3 px-3 py-2 rounded-lg border text-left transition-all ${
          ...
          >
            {ch.logo
              ? <img src={ch.logo} alt="" className="w-7 h-7 rounded object-contain bg-black flex-shrink-0" onError={e => e.target.style.display='none'} />
              : <div className="w-7 h-7 rounded bg-niceborder flex items-center justify-center text-xs flex-shrink-0">📺</div>
            }
